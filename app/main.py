...
st.title("Dashboard Elezioni Regionali 2024 - Genova")
...
colore = st.sidebar.color_picker("Colore", "#2563eb")
opacita = st.sidebar.slider("Opacità", 0.0, 1.0, 0.6)

# Mostra mappa + grafici
if mappa_tipo == "Municipi":
    st.subheader("Mappa dei Municipi")
    ...
    st.subheader("Mappa delle Sezioni")
    if tipo_visualizzazione == "Folium":
    ...
    st.subheader("Mappa delle Unita Urbanistiche")
    if tipo_visualizzazione == "Folium":
...
st.markdown("---")
st.markdown("Dashboard a cura del Comitato Elettorale")
